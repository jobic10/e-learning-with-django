    ...
    def process_view(self, request, view_func, view_args, view_kwargs):
        ...
        if user.is_authenticated:
            if user.user_type == '1':  # Admin
                if modulename == 'staff.views' or modulename == 'student.views':
                    messages.error(
                        request, "You do not have access to this resource")
                    return redirect(reverse('adminDashboard'))
            elif user.user_type == '2':  # Staff
                if modulename == 'administrator.views' or modulename == 'student.views':
                    messages.error(
                        request, "You do not have access to this resource")
                    return redirect(reverse('staffDashboard'))
            elif user.user_type == '3':  # Student
                if modulename == 'administrator.views' or modulename == 'staff.views':
                    messages.error(
                        request, "You do not have access to this resource")
